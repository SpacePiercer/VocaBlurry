...
- **Forgotten-word tracking** (`drain_updates.py`) — tapping 🤔 queues a Telegram
  ...
  webhook) and logs the words you blanked on. Taps are deduped and the button
  locks to ✅ once recorded.
- **Daily quiz** (`send_quiz.py`) — one quiz poll per word seen today.
...
- `drain_updates.py` — drains 🤔 taps into `data/review/forgotten.json`. Must be
  the bot's only `getUpdates` consumer.
- `send_quiz.py` — sends the end-of-day quiz polls.
